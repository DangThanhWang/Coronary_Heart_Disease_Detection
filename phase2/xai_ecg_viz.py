...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Unknown vs Prototype ECG templates.")
    parser.add_argument("--csv-root", type=Path, default=Path("Data") / "Generated_HardOOD2")
    parser.add_argument("--explain-csv", type=Path, default=Path("artifacts") / "phase2" / "xai_ecg_unknown_explain.csv")
    ...
    for unknown_file in unknown_files:
        ...
        for _, row in rows.iterrows():
            ...
            ax.plot(unk_template, label=f"Unknown", linewidth=2)
